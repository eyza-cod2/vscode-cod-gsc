...
class GscFileParser {
    ...
    static group(tokens) {
        ...
        function solve_unsolved(group, parentGroup = undefined, lastFunctionScope = undefined, lastDeveloperScope = undefined) {
            for (var i = 0; i < group.items.length; i++) {
                const innerGroup = group.items[i];
                solve_unsolved(innerGroup, group, group.type === GroupType.FunctionScope ? group : lastFunctionScope, group.typeEqualsToOneOf(GroupType.DeveloperBlock, GroupType.DeveloperBlockInner) ? group : lastDeveloperScope);
            }
            switch (group.type) {
                case GroupType.Root:
